...
- `src/vision/image_inspection_runner.py` now connects a real image through `MeasurementAdapter -> Observation -> RuleEngine -> InspectionOrchestrator` for offline replay.
- `config/Rule.machine_vision_test.cmd` is the isolated one-region replay recipe; it is not a production recipe.
- `tools/run_machine_vision_replay.py` and `RUN_MACHINE_VISION_REPLAY.bat` run the offline real-image path without camera or PLC hardware.
- `RuleEngine` now supports `MACHINE_VISION_MEASUREMENT` with optional measurement thresholds in Rule.cmd. This is an image-quality/measurement boundary, not component or grease classification.
