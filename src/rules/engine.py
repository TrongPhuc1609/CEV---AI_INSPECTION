...
class RuleEngine:
    ...
    def evaluate(self,obs:Observation)->Observation:
        ...
        method=str(rule.get("method",obs.method));obs.method=method;min_conf=rule.get("min_confidence")
    ...
    def _evaluate_component(self,obs,rule):
        expected=rule.get("expected_component");expected_qty=rule.get("expected_quantity");obs.expected_class=expected;obs.expected_quantity=expected_qty;counts=obs.metadata.get("class_counts",{})
        ...
        if rule.get("position_check"):
            tolerance=rule.get("position_tolerance_px");actual=obs.position;ex=rule.get("expected_position_x");ey=rule.get("expected_position_y")
            if tolerance is not None and ex is not None and ey is not None and actual:
                dx=float(actual.get("x",0))-float(ex);dy=float(actual.get("y",0))-float(ey);distance=(dx*dx+dy*dy)**0.5;obs.metadata["position_distance_px"]=distance
