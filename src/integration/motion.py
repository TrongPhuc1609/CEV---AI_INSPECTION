...
class MotionSafetyMonitor:
    ...
    def __init__(self, nominal_velocity: float, min_velocity: float, max_velocity: float,
                 camera_to_reject_distance: float, reject_max_ms: float = 200.0):
        ...
        self.reject_max_ms = float(reject_max_ms)

    def assess(self, velocity: Optional[float]) -> MotionAssessment:
        errors: list[str] = []
        if velocity is None:
            errors.append("VELOCITY_UNAVAILABLE")
            return MotionAssessment(None, False, None, False, tuple(errors))
        ...
        if self.camera_to_reject_distance > 0 and abs(velocity) > 0:
            ...
            window_ok = window_ms >= self.reject_max_ms
            if not window_ok:
                errors.append("REJECT_WINDOW_TOO_SHORT")
        return MotionAssessment(velocity, velocity_ok, window_ms, window_ok, tuple(errors))
