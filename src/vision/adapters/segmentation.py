...
class SegmentationAdapter(VisionAdapter):
    ...
    def inspect(self,data:VisionInput)->Observation:
        ...
        return Observation(data.product_id,data.inspection_id,data.region_id,self.method,detected_class=raw.get("class","grease"),confidence=raw.get("confidence"),coverage_percent=raw.get("coverage_percent"),metadata={"frame_id":data.frame_id,"raw":raw,"mask":raw.get("mask"),"target_zone_coverage_percent":raw.get("target_zone_coverage_percent"),"forbidden_zone_violation":bool(raw.get("forbidden_zone_violation",False))})
